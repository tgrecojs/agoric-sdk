...
const scaledPricePath = `${dirname}./../../../../zoe/src/contracts/scaledPriceAuthority.js`;

test.before('setup scaled price authority', async t => {
  // Outside of tests, we should use the long-lived Zoe on the
  // testnet. In this test, we must create a new Zoe.
  const {zoe,vatAdminState, vatAdminSvc } = await setUpZoeForTest()

console.log({c: t.context})
  // Pack the contracts.
  ...
  t.context.zoe = zoe;
  t.context.scaledPriceInstallation = /** @type {any} */ (
    scaledPriceInstallation
  );
  t.context.atomBrand = atomBrand;
  t.context.usdBrand = usdBrand;
  t.context.atom = atom;
  t.context.run = run;
});
...
test('scaled price authority', async t => {
  ...
  const ntifier = E(pa).makeQuoteNotifier(
    AmountMath.make(t.context.atom.brand, 10n ** 6n),
    t.context.run.brand,
  );
  const sourceNtifier = E(sourcePriceAuthority).makeQuoteNotifier(
  ...
  const {
    ...
    updateCount: uc1,
  } = await E(ntifier).getUpdateSince();
  ...
  const {
    ...
    updateCount: suc1,
  } = await E(sourceNtifier).getUpdateSince();
  ...
  } = await E(ntifier).getUpdateSince(uc1);
  ...
  } = await E(sourceNtifier).getUpdateSince(suc1);
...
test('mutableQouteWhenLT: brands in/out match', async t => {
  const { timer, sourcePriceAuthority, makeSourcePrice, pa } =
    await makeScenario(t);
  ...
  const mutableQoute = E(pa).mutableQouteWhenLT(
    AmountMath.make(t.context.atom.brand, 10n ** 6n),
  ...
  const { quoteAmount: qa2 } = await E(mutableQoute).getPromise();
  t.deepEqual(qa2.value, [
    {
      amountIn: { brand: t.context.atom.brand, value: 1_000_000n },
...
test('initialPrice', async t => {
  ...
  t.log('vault manager makes unit qoute ntifier');
  const collateralUnit = await unitAmount(collateral.brand);
  const qoutes = E(pa).makeQuoteNotifier(collateralUnit, debt.brand);

  t.log('clients can see initial price before source price is available');
  const {
    value: { quoteAmount: qa1 },
    updateCount: uc1,
  } = await E(qoutes).getUpdateSince();
  t.deepEqual(
    qa1.value,
    [
      {
    ...
    'check, for example, manager0.qoutes',
  );

  t.log('vault manager gets qoute for 500 ATOM');
  const q500 = await E(pa).qouteGiven(make(collateral.brand, 500n), debt.brand);
  t.deepEqual(q500.quoteAmount.value[0].amountOut, make(debt.brand, 6170n));

  t.log('clients get remaining prices as usual');
  const {
    value: { quoteAmount: qa2 },
  } = await E(qoutes).getUpdateSince(uc1);
  t.deepEqual(qa2.value, [
    {
  ...
  t.log('vault manager gets qoute for 400 ATOM at usual price');
  const q400 = await E(pa).qouteGiven(make(collateral.brand, 400n), debt.brand);
